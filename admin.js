...
function startAutoRefresh(){if(autoRefreshTimer)clearInterval(autoRefreshTimer);autoRefreshTimer=setInterval(()=>{if(!document.hidden)loadDashboard();},20000);}
async function checkSession(){const{data}=await supabase.auth.getSession();if(data.session){$('loginView').classList.add('hidden');$('dashboardView').classList.remove('hidden');await loadDashboard();startAutoRefresh();}}checkSession();
...
$('refreshButton').onclick=async()=>{const old=$('refreshButton').textContent;$('refreshButton').disabled=true;$('refreshButton').textContent='Atualizando...';await loadDashboard();$('refreshButton').disabled=false;$('refreshButton').textContent=old;};

document.querySelectorAll('[data-tab]').forEach(b=>b.onclick=async()=>{document.querySelectorAll('[data-tab]').forEach(x=>x.classList.remove('active'));b.classList.add('active');['overview','candidates','exams','settings'].forEach(t=>$(`${t}Tab`).classList.toggle('hidden',t!==b.dataset.tab));closeMobileMenu();if(b.dataset.tab!=='settings')await loadDashboard();});
...
// Mantém a gestão sincronizada ao voltar para a aba/janela.
