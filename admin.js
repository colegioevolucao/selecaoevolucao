...
document.querySelectorAll('[data-tab]').forEach(b=>b.onclick=async()=>{document.querySelectorAll('[data-tab]').forEach(x=>x.classList.remove('active'));b.classList.add('active');['overview','candidates','exams','settings'].forEach(t=>$(`${t}Tab`).classList.toggle('hidden',t!==b.dataset.tab));if(b.dataset.tab!=='settings')await loadDashboard();});
